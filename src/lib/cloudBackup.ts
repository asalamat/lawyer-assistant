...
// Each check on the combined "google-drive"/"onedrive" discriminant is
// grouped into one `if` — TypeScript only fully narrows DriveBackupConfig
// out of the union once both of its discriminant values have been excluded
// within the same control-flow branch, not across two separate sequential
// `if` statements.
