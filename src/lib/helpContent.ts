    ...
    items: [
      {
        ...
        name: "Leads",
        detail:
          "A board (nav > Leads) for tracking prospective clients before a matter exists — the intake questionnaire and everything else in this app assumes a matter is already open, this is the layer before that. Add a lead with just a name (email/source optional), then move it through New → Contacted → Consultation scheduled → Proposal sent → Won/Lost using the dropdown on its card or its own detail page, or delete one entirely from its detail page (type DELETE to confirm — permanent, no undo). When it's ready, \"Convert to matter\" on the lead's detail page creates a real matter (and client, if one doesn't already exist) the same way the normal new-matter flow does — the lead's own record stays intact afterward with a link to the matter it became, marked Won automatically. \"Get embed code\" gives an `<iframe>` snippet for the firm's own public website — a visitor who fills it in and submits lands here as a new lead automatically, tagged \"website,\" no login or app access involved on their end. That form is rate-limited per visitor to guard against spam.",
      },
      {
        slug: "sticky-notes",
        name: "Sticky notes",
      ...
      },
      {
        ...
        detail:
          "A firm-wide calendar (nav > Calendar) and a Calendar tab on every matter show the same underlying data from two angles: every deadline with a due date, plus any ad-hoc event you add directly (title, date, optional matter, optional reminder lead time in days). This is a fully native calendar — there's no Google/Microsoft account to connect and nothing external to keep in sync. Click a day to see its items or add a new event to it; delete an event (not a deadline — remove those from the matter's Deadlines tab instead) from the day's detail panel. An hourly check looks for deadlines due tomorrow or today and events entering their reminder window, and raises a reminder through up to three channels: the bell icon in the top bar (click to see recent reminders, mark one or all read), an email to every active staff account if SMTP is configured (Settings > Email), and a browser push notification on any device that opted in (Settings > Security > Browser notifications) — each reminder is only ever raised once per item, so re-checking never spams the same due date twice.",
      },
    ],
  },
  {
    ...
    items: [
      ...
      },
      {
        ...
        detail:
          "A matter's \"Consent & signatures\" tab prepares retainer agreements, conflict waivers, privacy consents, or any other document that needs the client's signature, and tracks each one through draft > awaiting signature > signed/declined/voided. Optionally attach one of the matter's uploaded documents as the thing being signed — the signature record then stores that file's content hash, so it's provable later which exact version was agreed to. \"Send for signature\" issues a single-purpose link (/sign/…): no login, no account, scoped to that one document, and it expires on its own after two weeks. If the matter has a client email on file and SMTP is configured (Settings > Email), that link is emailed to the client automatically — the tab also shows the link itself with a \"Copy link\" button as a backup, or for when there's no email on file to send it another way. The link is only shown at the moment it's issued — if it's lost, \"Resend link\" mints a fresh one (and re-sends the email) while the previous link stops working immediately. If the client has portal login access, the pending request also shows up automatically under \"Needs your signature\" in their portal — nothing further to send in that case. The signing page shows the document title and the client's name, takes their typed full legal name, an optional drawn signature, and an explicit \"I intend this as my legal signature\" confirmation; their IP address, browser, and timestamp are recorded alongside it. You can also mark a document declined (if the client says no by phone or email) or void it entirely, both of which kill any outstanding link. Every step is recorded in the audit log — signatures submitted through the client link show no attributed user, since the client isn't a user of this app. The same signing mechanism is also available from a matter's Document templates tab (\"Send for signature\" on a generated document) and from an invoice's \"Request client approval\" button on the Timesheet tab. Note this is a basic electronic signature, not a certificate-backed qualified/advanced e-signature. If DocuSign is configured and turned on (Settings > DocuSign), every one of these \"Send for signature\"/\"Request client approval\" actions routes through DocuSign automatically instead — DocuSign emails the client and hosts the entire signing ceremony on its own site, so there's no copy-link step and no local /sign/… link at all. That's specifically for setups where this app has no public URL of its own — DocuSign only needs outbound access to its API, no inbound access to this app. Status updates (signed/declined/voided) come back via a background check every few minutes, not instantly, since there's no webhook endpoint for DocuSign to call.",
      },
      {
      ...
      },
      {
        ...
        name: "Email",
        detail:
          "Compose and send an email directly from a matter once SMTP is configured in Settings. Use Smart draft to describe what the email should say and get a grounded subject+body draft citing the matter documents it's based on — always review before sending. Translate the message into another language right in the compose box (defaults to whatever's set in Settings > Translation) (replaces the draft in place so you can review and send the translated version). Attach any of the matter's own uploaded documents when sending. To bring a message FROM a connected inbox INTO this matter instead, see \"Import an email\" on the Overview tab, alongside document upload — that's a different direction (in, not out) so it lives with the rest of document intake rather than here.",
      },
    ],
  },
  {
    ...
    items: [
      ...
      },
      {
        ...
        name: "Templates",
        detail:
          "For routine documents you send the same way every time — a standard retainer letter, a form notice — instead of an AI draft from scratch. Anyone can author a reusable template (Settings > Document templates) as plain text with {{field}} placeholders; matter.title, matter.fileNumber, matter.matterType, matter.clientName, client.email, client.phone, today, and lawyerName fill in automatically, and anything else you write as a placeholder becomes a field you type a value for when generating. A matter's own Templates tab picks a template, prompts for just the custom fields it needs, and generates a filled-in copy in seconds — translate it, export it as PDF or a real .docx file, or \"Save as document\" to add it to the matter's own document list (so it can be attached to an outgoing email like anything else). Deleting a template later doesn't remove documents already generated from it — each one is a complete, standalone copy at the moment it was generated.",
      },
      {
        slug: "ai-redundancy",
  ...
  },
  {
    ...
    items: [
      ...
      },
      {
        ...
        name: "DocuSign",
        detail:
          "Admin-only. Optional — this app's own e-signature system (Consent tab, Document templates, invoice approval) already works with no setup at all. Turn this on only if you specifically want DocuSign instead: it emails the client directly and hosts the entire signing ceremony on its own site, which is the only way signing can work if this app has no public URL of its own (running on one local machine). Setup, step by step: (1) Sign in to your DocuSign developer/sandbox account and go to https://admindemo.docusign.com/api-integrator-key (use this exact demo-environment URL, not account.docusign.com or admin.docusign.com — those manage a separate production app registration). (2) Find or create an app there and copy its Integration Key. (3) Click \"Generate RSA\" — this produces a public/private keypair; copy the private key that starts with -----BEGIN RSA PRIVATE KEY----- (not the public key, which starts with -----BEGIN PUBLIC KEY----- and won't work here). (4) Under \"Additional settings\" on that same page, click \"Add URI\" (a button that adds a new empty box specifically for a redirect URI — don't type into the nearby \"Link to Privacy Policy\"/\"Link to Terms of Use\" fields, those are unrelated), enter https://www.docusign.com into that new box, then click the page's own Save/Update button and reload the page to confirm it's still listed afterward. (5) Copy the User ID and Account ID shown under \"My Account Information\" in the same DocuSign account. (6) Back in this app, enter the Integration Key, User ID, Account ID, and private key here, check \"Route e-signature requests through DocuSign,\" and Save. (7) This page then shows a one-time admin-consent URL — open it in a browser while logged into the same DocuSign account and click Allow; this only needs to be done once per integration key, and it's what \"There are no redirect URIs registered\" or similar errors mean if it's skipped or the URI from step 4 didn't actually save. (8) Click \"Test connection\" to confirm everything resolves correctly. Once enabled, sending flips automatically for every existing e-signature action — no per-document choice needed. Since this app has no inbound URL for DocuSign to notify directly, a background check every 5 minutes (not instant) looks for newly completed envelopes, pulls the signed document back in, and records the signature — a change to this polling logic itself (unlike most of this app) needs a full app restart to take effect, not just a page refresh, since it runs as a long-lived background timer rather than something re-loaded per request.",
      },
      {
      ...
      },
      {
        ...
        name: "Backup & restore",
        detail:
          "Admin-only. \"Backup now\" saves the entire app — matters, documents, clients, users, settings — into one downloadable file; the last 10 are kept automatically. Every download is recorded in the audit log, and downloading many backups within the same hour is rate-limited (with a flagged alert before the hard limit kicks in) — a backup is the single most sensitive export this app can produce, so it's worth noticing if it happens a lot in a short window. Restoring replaces all current data with a backup's contents (current data is moved aside on disk, not deleted, but you must restart the app right after for the restore to take effect) — typing \"RESTORE\" is required to confirm. \"Restore from a file\" does the same thing from any backup .tar.gz browsed from this computer's filesystem, not just the ones kept in the list above — one downloaded earlier, copied in from another machine, or from before a migration." +
          " Two independent automatic-backup mechanisms run inside the app itself — no OS-level cron job required (though the page still shows the command for anyone who prefers wiring one up separately): a fixed-interval schedule (e.g. hourly), and an activity-triggered one that waits for a configurable quiet period after real changes in the app before backing up, capped by a minimum cooldown so continuous use can't trigger back-to-back runs. Either or both can be on." +
          " Cloud storage is optional and provider-agnostic: any S3-compatible bucket (AWS S3, Cloudflare R2, Backblaze B2, Wasabi, MinIO — just a bucket + access key, no app registration), Google Drive or OneDrive via OAuth (reuses the same Client ID/Secret already entered for the email integration in Settings > Integrations), or rclone — the only path needing zero app registration at all, since rclone ships its own. An in-app setup wizard drives rclone's own OAuth flow end to end: it can install rclone itself (via Homebrew/winget) if missing, then walks through connecting an account with a single \"Start\" click — the only manual step left is signing in and approving access when the browser opens, exactly like any \"Sign in with Microsoft/Google\" button. Google Drive and OneDrive backups only ever touch a dedicated folder they create themselves, never the rest of the connected account." +
          " Note: the encryption key isn't included in the backup (it lives in the macOS Keychain, or a separate file on Windows/Linux) — back that up separately too, or a restored backup's secrets/documents can't be decrypted.",
      },
    ],
  },
];
