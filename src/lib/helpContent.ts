    ...
    items: [
      ...
      },
      {
        ...
        detail:
          ...
          "- From the main Trust accounting page, **\"Reconcile\"** compares the account's ledger total *as of the statement date* against a bank statement balance you enter, recording the comparison (and any variance) permanently, whether or not it matches. It also snapshots every matter's own trust balance as of that same date — the **three-way check** most bars require monthly (bank = ledger = sum of individual client balances) — expand **\"Client trust ledger\"** under a past reconciliation to see that per-client listing\n" +
    ...
    items: [
      ...
      },
      {
        ...
        detail:
          ...
          "- **Cloud storage** is optional and provider-agnostic: any S3-compatible bucket (AWS S3, Cloudflare R2, Backblaze B2, Wasabi, MinIO), or Google Drive/OneDrive via direct sign-in — a one-time admin app registration (separate from any email integration, done once in Settings > Backup), then every staff member just signs in with their own Google/Microsoft account, nothing to paste. Google Drive/OneDrive backups only ever touch a dedicated folder they create themselves\n" +
