...
export const HELP_SECTIONS: HelpSection[] = [
  {
    ...
    items: [
      ...
      },
      {
        ...
        detail:
          "Drag and drop files onto a matter. Text, PDF, Word (.docx), Excel/CSV, images (via OCR), and audio/video recordings (via OpenAI Whisper, once configured) are readable by chat and AI features — other file types still upload but aren't used as AI context. Dropping a .zip (e.g. a disclosure package or a folder someone zipped up) unpacks it and uploads each file inside individually — useful for bulk intake instead of uploading one at a time; a per-file result list shows what succeeded or failed. Identical files uploaded twice are flagged as duplicates; near-identical content (e.g. the same letter re-scanned or re-saved in another format) is separately flagged as a near-duplicate, based on how similar the extracted text is, not the file bytes. If a document can't be read at all (a corrupt PDF, an unsupported encoding), it shows an \"extraction failed\" badge with the real error on hover and a Retry button, instead of silently vanishing from AI context. A chat-readable document also shows its detected language (if not English) and a quality score when it's below 70% — low quality usually means a scanned image OCR'd with low confidence, or unusually little text for the file, worth a manual look. Every upload is scanned for malware first (via a local ClamAV install, Settings > Privacy — no cloud account, nothing leaves the machine); a flagged file is quarantined to a separate location on disk instead of being stored where AI features or chat can read it, and shows a \"quarantined\" badge instead of the normal upload result. Documents are encrypted at rest and decrypted transparently when read. Click \"Similar\" next to a chat-readable document to find other documents in the same matter (including attached reference-library material) with related content, ranked by similarity — useful for spotting related correspondence or content that overlaps without being a near-duplicate.",
      },
      {
        slug: "notes",
        name: "Notes",
