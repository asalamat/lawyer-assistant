    ...
    items: [
      ...
      },
      {
        ...
        detail:
          ...
          "- **Cloud storage** is optional and provider-agnostic: any S3-compatible bucket (AWS S3, Cloudflare R2, Backblaze B2, Wasabi, MinIO), Google Drive or OneDrive (direct sign-in, own dedicated app registration — a one-time admin setup, separate from any email integration), or **rclone** — the only path needing zero app registration at all. An in-app setup wizard drives rclone's own OAuth flow end to end, including installing rclone itself if missing. Google Drive/OneDrive backups only ever touch a dedicated folder they create themselves, and staff connecting an account afterward just sign in with their own Google/Microsoft account — no credentials to paste\n" +
