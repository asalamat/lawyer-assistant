...
export async function listStickyNotes(userId: string, pagePath: string): Promise<StickyNote[]> {
  return db
    .prepare(
      "SELECT id, pagePath, content, color, createdAt, updatedAt FROM sticky_notes WHERE userId = ? AND pagePath = ? ORDER BY createdAt ASC",
    )
    .all(userId, pagePath)
...
export async function addStickyNote(
  ...
  db.prepare(
    "INSERT INTO sticky_notes (id, userId, pagePath, content, color, createdAt, updatedAt) VALUES (?, ?, ?, '', ?, ?, ?)",
  ).run(id, userId, pagePath, color, now, now);
  return { id, pagePath, content: "", color, createdAt: now, updatedAt: now };
}

export async function updateStickyNote(
  userId: string,
  id: string,
  updates: { content?: string; color?: StickyNoteColor },
  ...
  db.prepare("UPDATE sticky_notes SET content = ?, color = ?, updatedAt = ? WHERE id = ?").run(
    content,
    color,
    updatedAt,
    id,
  );
  return { id, pagePath: existing.pagePath, content, color, createdAt: existing.createdAt, updatedAt };
}
